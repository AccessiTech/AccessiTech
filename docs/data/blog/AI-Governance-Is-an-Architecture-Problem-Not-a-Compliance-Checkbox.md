...
visual_notes: |
  ...
  Inline—Policy vs. Substrate Comparison (~800×400 px): Two-column table rendered as image. Left: "Policy Document" structure with lock icon (locked=cannot change). Right: "Governance Substrate" structure with checkmark (enforced=behavior). Tool: Excalidraw or Figma. WCAG-AA (≥14px, 4.5:1 contrast).
  Accessibility: All text ≥14px for legibility, 4.5:1 contrast minimum (white/light text on dark background preferred for governance tone). Include descriptive alt text for each image.
-->
...
![Timeline arc showing the incident pattern across industries: cascading blocks from Deployment through Policy Signed, System Runs, Operates Outside Scope (with unknown origin bracket), Incident Surfaces, to Legal Response](/assets/images/be-1-timeline-arc.png)
...
![Graph titled 'AI Harnesses reduce Value Divergence by Covering the Governance Gap' showing two series over time: dark blue 'Vibe Coding' declining after Specs Only phase, yellow Endogenous Agentic Design rising steadily through Values/Specs/Scripts phases to high plateau, with dotted bracket showing AI Agent Governance Gap](/assets/images/be-1-divergance-graph.png)
...
> "If you use a closed harness, especially if it's behind an API, you don't own your memory." — Harrison Chase, CEO of LangChain, ["Your Harness, Your Memory (April 2026)"](https://www.langchain.com/blog/your-harness-your-memory)
...
> The harness exists in the colorful layers between your policy docs and your actual digital product / service. If you don't own the harness, you don't own the governance.
