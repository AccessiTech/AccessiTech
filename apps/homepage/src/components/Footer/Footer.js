...
function Footer () {
  return (<section className="footer-section" aria-label={useT(FOOTER)}>
    ...
    <p aria-label={useT(REPORTING_AN_ISSUE)} className='github-issues'>
      ...
      <a
        ...
        target={TARGET_BLANK}
        alt={GITHUB_ISSUES}
      >{useT(GITHUB_ISSUES)}</a>!
    </p>
    <p aria-label={useT(COPYRIGHT)} className="copyright">{useT(COPYRIGHT_P1)}</p>
  </section>);
}
