...
date: 2026-05-18
status: published
og_image: assets/images/be-5-og.png
og_image_alt: Introducing EndogenAI/DogmaMCP
image: assets/images/be-5-og.png
imageAlt: A graphic depictiving a plan sprouting from a book with roots growing as a network of nodes, depicting the concept of products/services cultivated from org values.
-->
...
🌍 This post introduces a framework designed to close that gap. [EndogenAI](https://github.com/EndogenAI) is the open source community of practitioners advancing values-aligned AI governance. [dogma](https://github.com/EndogenAI/dogma) is the framework that encodes that methodology into rules your AI reads and follows. [DogmaMCP](https://github.com/EndogenAI/dogma/tree/main/mcp_server) is the experimental software tooling that puts those rules into practice as your AI works — where accountability isn't a promise, but an audit trail.
...
<!-- graphic in progress: vendiagram or shapes depicting syndersitc strengths and weaknesses; maybe a set of triangles showing how the two sides complement each other and how governance brings out the synergy -->

Most existing AI governance solutions focus on constraining single models at the point of output—either external dashboards (Software/Service compliance tools) or local guardrails scattered across scripts and pre-commit hooks. That's useful, but it leaves a gap at the coordination layer. When multiple AI agents work together, what matters most is how they coordinate: what information they can access, who has authority to review their decisions, and how their combined behavior gets audited. EndogenAI's dogma is designed to operate at that coordination layer. Open-source infrastructure remains the only coherent model for governance tooling—proprietary platforms lock organizations into their interpretation of your values.

<!-- visual depicting the two different substrates, the first can be like paper documents, the second like automated checks with a code symbol or something -->

The first surface is the policy substrate: **the foundational layer where your values are encoded as operational constraints.** Not aspirational language in a PDF, but specific rules about how systems behave, what data they access, and what decisions require human oversight.

The second surface is the enforcement substrate: **the tooling that checks compliance automatically, at development time, before anything ships.** Think basic unit tests, continuous integration hooks, runtime validators, audit trails—mechanisms, etc. - that don't require human vigilance every single time.

<!-- visual depicting a user interacting with a computer in some sort of a cycle, we could probably look for inspiration with some real standard HCI and cyclical iconography -->
...
<!-- maybe here we can reuse a visual about the divergence and values drift from the previous blog post -->
...
<!-- a visual of community holding up a globe or something -->
...
The community is the backbone. EndogenAI practitioners are researchers, engineers, and compliance professionals who've faced the same gaps—policies that don't translate to code, enforcement that breaks under load, audit trails that don't exist until something goes wrong. The methodology emerges from that shared experience. It's not theorized into existence; it's refined in production, across teams, documented as it stabilizes. The framework encodes what worked and what didn't.

<!-- a visual or maybe icon of a book, maybe a holy book but without any holy symbols -->

Your [dogma repository](https://github.com/EndogenAI/dogma) is where your organization encodes its values in two key files. [MANIFESTO.md](https://github.com/EndogenAI/dogma/blob/main/MANIFESTO.md) is your constitution — it captures the foundational principles you believe in and won't compromise on. [Agent files](https://github.com/EndogenAI/dogma/tree/main/.github/agents) are your operations manual — it translates those principles into specific instructions your AI systems read and follow. These aren't aspirational documents gathering dust; **they're the rules the system actually enforces.**

<!-- a visual of gears and wrench and circuitry or something -->
...
This synthesis integrates established patterns from three domains: [Constitutional AI (Anthropic, 2023)](https://arxiv.org/abs/2212.08073) covers LLM alignment via principle-driven constraints; [policy-as-code practices (HashiCorp OPA, Kyverno)](https://www.openpolicyagent.org/) establish governance as verifiable code; and [GitOps discipline (Weaveworks, FluxCD)](https://opengitops.dev/) operationalize versioned infrastructure enforcement. None of these individually solve the full governance challenge.

> EndogenAI combines them into the first integrated framework that brings philosophy-driven constraints, testable policies, and local enforcement together as a unified system.
...
🌱 Another way to think about it: governance grows like a seedling from your values. The seed is your MANIFESTO.md—immutable principles. The roots anchor deeper as each session adds documented constraints and validated patterns. The environment is your team, your tooling, the AI capabilities you choose to engage. **Each session with dogmaMCP adds growth rings to that substrate**—accumulated knowledge that future AI sessions inherit automatically, without re-discovering it from scratch. The system gets more reliable over time because learning gets encoded, not forgotten. And like a plant that outgrows its first pot, your governance framework stays under your control even as it scales across teams and projects.

In [EndogenAI/dogma](https://github.com/EndogenAI/dogma), this becomes a five-layer encoding chain:
...
**Example: MANIFESTO.md axiom** — _Your organization's foundational commitments, machine-readable_
...
**Example: pre-commit hook validation output** — _Every commit is checked against your rules before it lands_
...
**Example: Audit trail in git log** — _Governance decisions traceable back through your entire history_
...
<!-- a visual showing like cloud based versus local based processes where in the cloud gets disconnected but the computer is still connected -->
...
<!-- a visual comparing a magnifying glass inspecting a document to a telescope looking up at the clouds -->
